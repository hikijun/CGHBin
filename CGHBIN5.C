...
#define   TimeFactor    4.25e-3              /* Time requierment for a point */
...
int    bits[2] = { b11110000, b00001111 };
...
main()
{
   char      InputFile[14], OutputFile[14];
   ...
   double    CarBandwidth;
   double    ObjBandwidth, AptrBandwidth;
   ...
   if ( CarBandwidth < ObjBandwidth)          /* Minimum sampling rate       */
   {
       ...
       ySamplingRate = CarBandwidth;
   }
   else
   {
       xSamplingRate = ObjBandwidth * SampleRateMod;
       ySamplingRate = ObjBandwidth;
   }
      ...
      for(i=0 ; i < xSamples ; i++)
      {
         ...
         for(j=0 ; j < yBytes ; j++)
         {
            ...
            for(jj=0 ; jj < BitsInByte ; jj++)
            {
               ...
               aByte = (Eh > Threshold) ? aByte + bits[jj] : aByte + b00000000;
...
{
   aX = 0.1;                        /* Aperture size in x,y                  */
   aY = 0.1;
   WvLen = 632.8e-6;                /* A He-Ne LASER wavelength              */
   CarAngle = deg2rad(2.50);        /* Off axis carrier angle                */
   z0 = 100.0;                      /* Distance between object and hologram  */
   SampleRateMod = 0.250;           /* Sampling rate modify for carrier axis */
   page = 1;                        /* Page number for future evaluation     */
   xSamples = 2000;
   ySamples = 2000;
}
...
void Title(void)
{
   ...
   puts(" It will represent as binary intensity and produce a hologram ");
   puts(" bitmap.\n ");
   puts(" CghBin5xx.xxx ");
   puts(" Version 1.98  ");
   puts(" July 23rd - August 6th, 1990. ");
   puts(" Minor modify on November 9th, 1990.");
   puts(" ");
   puts(" ");
