/*
      ...
      Author   :   Kijun Park
               ...
               - This program will plot with HP printers for CGH bitmap.
*/
#include <stdio.h>
#include <stdlib.h>
...
main()
{
   ...
   {
      fprintf(stdprn,"\33*b%dW", yBytes);
      printf(".");
      for(j=0 ; j < yBytes ; j++)
         fputc(getc(FileInput), stdprn);
   }
   fprintf(stdprn, "\33*rB");

   fclose(FileInput);

   fprintf(stdprn, "\f\33E");
...
{
    fprintf(stdprn, "\33E");
    fprintf(stdprn, "\33*p%dx%dY", x, y);         /* Set pen position       */
    fprintf(stdprn, "\33*t%dR", dpi);             /* Set Resolution in DPI  */
    fprintf(stdprn, "\33*r1A");               /* Plot from current position */
}
